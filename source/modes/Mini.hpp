    ...
    virtual void update() override {
        ...
        if (isActive("CPU")) {
            ...
            if (settings.showCPUTemp)
                snprintf(mini_cpu_temp_c, sizeof(mini_cpu_temp_c), "%d\u2103", (int)(componentCPU_mC / 1000));
            else
                mini_cpu_temp_c[0] = '\0';
        }
        ...
        if (isActive("GPU")) {
            ...
            if (settings.showGPUTemp)
                snprintf(mini_gpu_temp_c, sizeof(mini_gpu_temp_c), "%d\u2103", (int)(componentGPU_mC / 1000));
            else
                mini_gpu_temp_c[0] = '\0';
        }
        ...
        if (isActive("RAM")) {
            // Build RAM bandwidth string (3 significant figures) when enabled.
            ...
            if (settings.showRAMBandwidth) {
                if (ramBW_MBs > 0) {
                    if (ramBW_MBs >= 10000) {
                        ...
                        snprintf(MINI_RAM_bw_c, sizeof(MINI_RAM_bw_c), "%u.%02u GB/s", bwInt, bwDec);
                    } else {
                        // < 1 GB/s: 2 decimal places, leading "0."
                        const unsigned bwDec = ramBW_MBs / 10;
                        snprintf(MINI_RAM_bw_c, sizeof(MINI_RAM_bw_c), "0.%02u GB/s", bwDec);
                    }
                } else {
                    snprintf(MINI_RAM_bw_c, sizeof(MINI_RAM_bw_c), "0.00 GB/s");
                }
            }
        ...
        if (isActive("RAM") && settings.showRAMTemp)
            snprintf(mini_ram_temp_c, sizeof(mini_ram_temp_c), "%d\u2103", (int)(componentRAM_mC / 1000));
        else
            mini_ram_temp_c[0] = '\0';
