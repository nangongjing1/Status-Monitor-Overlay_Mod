...
public:
    FullOverlay() { 
        ...
        //message = "按 " + formattedKeyCombo + " 退出";
        ...
        auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
            ...
            if (R_SUCCEEDED(clkrstCheck) || R_SUCCEEDED(pcvCheck) || R_SUCCEEDED(Hinted)) {
                ...
                if (R_SUCCEEDED(Hinted)) {
                    ...
                    renderer->drawString("总计\n应用\n小程序\n系统\n系统保留", false, COMMON_MARGIN, height_offset + 40, 15, renderer->a(settings.catColor2));
                    renderer->drawString(RAM_var_compressed_c, false, COMMON_MARGIN + valueOffset, height_offset + 40, 15, (settings.textColor));
                    renderer->drawString(RAM_percentage_var_compressed_c, false, ramPercentageOffset, height_offset + 40, 15, renderer->a(settings.textColor));
                }
            }
            
            ///Thermal
            if (R_SUCCEEDED(i2cCheck) || R_SUCCEEDED(tcCheck) || R_SUCCEEDED(pwmCheck)) {
                renderer->drawString("主板", false, 20, 550, 20, renderer->a(settings.catColor1));
                if (R_SUCCEEDED(i2cCheck)) {
                    //static auto batteryLabelWidth = renderer->getTextDimensions("Battery Power Flow: ", false, 15).first;
                    renderer->drawString("电池功率", false, COMMON_MARGIN, 575, 15, (settings.catColor2));
                    renderer->drawString(BatteryDraw_c, false, COMMON_MARGIN + valueOffset, 575, 15, renderer->a(settings.textColor));
                }
