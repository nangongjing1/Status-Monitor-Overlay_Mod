...
public:
                ...
                switch (key3) {
                    ...
                    case 0x52414D: // "RAM"
                        if (!(seen_flags & 4)) {
                            ...
                            renderItems.push_back({2, "内存", RAM_var_compressed_c, RAM_volt_c,
                    ...
                    case 0x544D50: // "TMP"
                        if (!(seen_flags & 16)) {
                            ...
                            renderItems.push_back({4, "温度", skin_temperature_c, SOC_volt_c, false});
                            seen_flags |= 16;
                        }
                        break;
                    case 0x524553: // "RES"
    ...
    virtual void update() override {
        ...
        if (settings.showCPUTemp)
            snprintf(cpu_temp_c, sizeof cpu_temp_c, "%d\u2103", (int)(componentCPU_mC / 1000));
        else
            cpu_temp_c[0] = '\0';

        if (settings.showGPUTemp)
            snprintf(gpu_temp_c, sizeof gpu_temp_c, "%d\u2103", (int)(componentGPU_mC / 1000));
        else
            gpu_temp_c[0] = '\0';

        if (settings.showRAMTemp)
            snprintf(ram_temp_c, sizeof ram_temp_c, "%d\u2103", (int)(componentRAM_mC / 1000));
        else
            ram_temp_c[0] = '\0';
